...
+ (NSString *)getDateStringFromtempString:(NSString*)tmp
{
    NSTimeInterval time = [tmp doubleValue];
...
+ (NSInteger)getYearOfDate:(NSDate *)date{
    NSCalendar *cal = [[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar];
    NSDateComponents *dateCom = [[NSDateComponents alloc] init];
    dateCom = [cal components:NSYearCalendarUnit fromDate:date];
    return [dateCom year];
}
 ...
 * @param sendDate 比较的日期
 */
+ (NSInteger)compareDay:(NSDate *)firstDate sendDate:(NSDate *)sendDate{
    //对日期进行格式化
    firstDate = [CommonUtil getDateForString:[CommonUtil getStringForDate:firstDate format:@"yyyy-MM-dd HH:mm:ss"] format:@"yyyy-MM-dd 00:00:00"];
    sendDate = [CommonUtil getDateForString:[CommonUtil getStringForDate:sendDate format:@"yyyy-MM-dd HH:mm:ss"] format:@"yyyy-MM-dd 00:00:00"];
