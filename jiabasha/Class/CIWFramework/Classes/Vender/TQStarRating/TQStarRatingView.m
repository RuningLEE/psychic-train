...
- (id)initWithFrame:(CGRect)frame numberOfStar:(int)number
{
    ...
    if (self) {
        ...
        self.starBackgroundView = [self buidlStarViewWithImageName:@"星星灰"];
        self.starForegroundView = [self buidlStarViewWithImageName:@"我的订单_选中"];
...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    CGFloat width = floor((CGRectGetWidth(frame) - _starSpace*(self.numberOfStar - 1))/self.numberOfStar);
    for (int i = 0; i < self.numberOfStar; i ++)
    {
        
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
        imageView.frame = CGRectMake(i * width + _starSpace*i, 0, width, frame.size.height);
