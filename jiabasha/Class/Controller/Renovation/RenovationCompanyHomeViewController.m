...
#import "Growing.h"
@interface RenovationCompanyHomeViewController ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,MFMailComposeViewControllerDelegate, TencentSessionDelegate>
{
    UIView * Viewfooter;
}
...
@property (nonatomic, strong) NSMutableArray *arrStoreCaseData; // 精彩案例，店铺相册
@property (nonatomic, strong) NSString *allCaseNum;
@end
//WonderfulCaseTableViewCell
@implementation RenovationCompanyHomeViewController
...
- (void)viewDidLoad {
    ...
    [self getBuildingStoreCommentDataPage:_pageComment];
}
// 精彩案例
...
- (void)setUiCornerRadiu{
    ...
    //设置头视图
    self.viewHead.frame = CGRectMake(0, 0, kScreenWidth, 292);
...
- (void)setHeadUi{
    ...
    self.companyStarView = [[TQStarRatingView alloc] initWithFrame:CGRectMake(0, 0, 63, 11) numberOfStar:5 starSpace:2];
    self.companyStarView.couldClick = NO;//不可点击
    [self.viewStar addSubview:self.companyStarView];
    [self.companyStarView changeStarForegroundViewWithPoint:CGPointMake([storeDetail.scoreAvg floatValue]/500*63, 0)];//设置星级
    ...
    self.viewHead.height = 292 + lookSignHeight -76 + viewCouponHeight -71;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;{
    ...
    if (tableView == self.tableView) {
        ...
        [cell.btnAllWatch addTarget:self action:@selector(btnWatchAllCase) forControlEvents:UIControlEventTouchUpInside];
         [cell.btnAllWatch setTitle:[NSString stringWithFormat:@"查看全部%@相册...",_allCaseNum] forState:UIControlStateNormal];
...
//    
//    UILabel *labelTitle = [[UILabel alloc] initWithFrame:CGRectMake(35, 0, kScreenWidth -10, 44)];
...
//    
//    UILabel *labelAn = [[UILabel alloc] initWithFrame:CGRectMake(10, 15, 16, 16)];
...
//    
//    UIView *lineBottom = [[UIView alloc] initWithFrame:CGRectMake(0, 44, kScreenWidth, 1)];
...
//    return headView;
    ...
    NSString * commmentNum = [NSString stringWithFormat:@"全部%@个相册",_allCaseNum];
    NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:commmentNum];
    ...
    [getProductStoreCateogry requestWithParameters:parameters
                                 ...
                                 onRequestFinished:^(CIWBaseDataRequest *request) {
                                     ...
                                     if ([RESPONSE_OK isEqualToString:request.errCode] || [@"OK" isEqualToString:request.errCode]) {
                                        ...
                                        [weakSelf.btnCheckAllShop setTitle:[NSString stringWithFormat:@"查看全部%@个商品...",request.resultDic[@"total"]] forState:UIControlStateNormal];
                                 }
                                 onRequestCanceled:^(CIWBaseDataRequest *request) {
